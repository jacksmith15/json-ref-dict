...
typecheck() {
    ...
    mypy --ignore-missing-imports json_ref_dict tests || EXIT_CODE=1
}

tests() {
    header "Running unit tests" "$blue"
    pytest -v -s --junitxml=unit_test_results.xml --cov="json_ref_dict" --cov-append --cov-branch --cov-report= tests || EXIT_CODE=1
}
